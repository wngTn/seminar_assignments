...
class ConstantOpLowering : public mlir::OpRewritePattern<relay::ConstantOp> {
  using OpRewritePattern<relay::ConstantOp>::OpRewritePattern;

  mlir::LogicalResult matchAndRewrite(relay::ConstantOp op, mlir::PatternRewriter &rewriter) const final {
...
namespace {
class RelayToAffineLowerPass : public mlir::PassWrapper<RelayToAffineLowerPass, mlir::FunctionPass> {
...
void RelayToAffineLowerPass::runOnFunction() {
...
std::unique_ptr<mlir::Pass> relay::createLowerToAffinePass() {
  return std::make_unique<RelayToAffineLowerPass>();
}