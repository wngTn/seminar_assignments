...
public:
  explicit PrintOpLowering(mlir::MLIRContext *context)
    : mlir::ConversionPattern(relay::PrintOp::getOperationName(), 1, context) {}
  ...
  mlir::LogicalResult matchAndRewrite(mlir::Operation *op,
    ...
    for (unsigned i = 0, e = memRefShape.size(); i != e; ++i) {
      ...
      if (i != e - 1)
        rewriter.create<mlir::CallOp>(loc, printfRef, rewriter.getIntegerType(32), newLineCst);
      rewriter.create<mlir::scf::YieldOp>(loc);
      rewriter.setInsertionPointToStart(loop.getBody());
    }

    // Generate a call to printf for the current element of the loop.
    auto printOp = mlir::cast<relay::PrintOp>(op);
...
namespace {
class RelayToLLVMLoweringPass : public mlir::PassWrapper<RelayToLLVMLoweringPass, mlir::OperationPass<mlir::ModuleOp>> {
public:
...
void RelayToLLVMLoweringPass::runOnOperation() {
  ...
  patterns.add<relay::PrintOpLowering>(&getContext());
...
std::unique_ptr<mlir::Pass> relay::createLowerToLLVMPass() {
