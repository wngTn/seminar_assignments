...
then
    ...
    Page size (bytes): %Z" \
    ./linear_regression-seq linear_regression_datafiles/key_file_50MB.txt \
    && \
    printf "\n" \
    && \
    ...
    Minor (reclaiming a frame) page faults: %R
    Page size (bytes): %Z" \
    ./linear_regression-seq linear_regression_datafiles/key_file_100MB.txt  \
    && \
    printf "\n" \
    &&
    ...
    Minor (reclaiming a frame) page faults: %R
    Page size (bytes): %Z" \
    ./linear_regression-seq linear_regression_datafiles/key_file_500MB.txt
