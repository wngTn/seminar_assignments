...
<u>Table 1</u> Relative code size compared with the respective sequential implementation

As expected, the applications that ran with multi-threaded implementations were all faster than their sequential versions, in every application, and every data size. However, both the <samp>PThread</samp> and <samp>MapReduce</samp> variants differ in few aspects, which we want to investigate further now:

- It is noticable in *Figure 1.1* that <samp>MapReduce</samp> has an inferior speed-up than the <samp>PThread</samp> implementations in every application. This is due to the fact that the whole execution time is compared.
<samp>MapReduce</samp> needs to setup splitter arguments, the <samp>MapReduce</samp> arguments and call the <samp>MapReduce</samp> scheduler. Therefore, if the input is small enough, this can create a lot of overhead, for the programs might need longer to manage and set everything up than the actual computation. Still, <samp>MapReduce</samp> is up to 5 times faster than the sequential version in most applications.

- Matrix Multiplication has always been a target for multi-threaded applications, since they are almost perfect to parallelize. It is no surprise that this application has one of the best speed-up (*Figure 1.1, 2.1*). The better cache-locality (*Figure 1.2, 2.2*) is due to the fact that the input matrix is still small enough to be placed at least in the L3-Cache (6 MiB). Additionally, more cores accomplish less cache-misses, for they need to access the memory less overall, compared to single-core programs. However, as in *Figure 3.1* and *Figure 3.2* shown, the performance is getting worse if the matrices are getting too big (Large dataset: 1000x1000). For they can not fully fit into the cache anymore and thus the cache-miss rate is getting higher, leading to a dropdown in speed-up from ~650% down to ~150%.

- The 7.5 speedup of the <samp>Word Count</samp> application (*Figure 3.1*) is due to its key-based structure, which <samp>MapReduce</samp> can benefit hugely from, making it also much faster than the <samp>PThread</samp> implementation. Even though, it is the application with the most cache-misses, it can amortize this overhead with its seperation between the mapping phase and reduce phase. 

- This key-based structure also fits well the <samp>String Match</samp> algorithm, which makes full use of the parallelization. While the cache-misses are similar to the sequential version, it ranges speedups up to 350% (*Figure 1.1, 2.1, 3.1*). Both the <samp>PThread</samp> and <samp>MapReduce</samp> implementation.

- Remarkable is the surprising MapReduce speedup in the <samp>PCA</samp> application with a large dataset (*Figure 3.1*), since the <samp>MapReduce</samp> implementation is behind the <samp>PThread</samp> implementation with small and medium datasets (*Figure 1.1, 2.1*). The surprising speedup is due to the large input size (4096x16384). The <samp>PThread</samp> implementation uses the original array structure for direct access, which can lead to an overhead if the arrays get too big, hence the <samp>PThread</samp> implementation has a worse cache-miss rate (*Figure 3.2*). Whereas the <samp>MapReduce</samp> implementation operates mostly on pointers and can therefore reduce this overhead.

- Even though the rest of the applications (<samp>Linear Regression, Kmeans, Histogram</samp>) achieve speedups up to ~300%, the <samp>PThread</samp> implementation and <samp>MapReduce</samp> implementation are comparable in performance, or the <samp>MapReduce</samp> implementation is even slightly worse. This is because these algorithms do not fit very well with the <samp>MapReduce</samp> program structure. For instance, the <samp>Kmeans</samp> algorithm calls the <samp>MapReduce</samp> scheduler iteratively, giving it a slight disatvantage against the <samp>PThread</samp> implementation. Further, in contrary to the <samp>MapReduce</samp> implementation, the <samp>PThread</samp> implementation of the <samp>Histogram</samp> application does not use keys as output format additionally, it omits the final sorting of the output data, unlike the <samp>MapReduce</samp> implementation, resulting in a greatly reduced overhead, better performance and better cache-locality (*Figure 1 - 3*)

- Multicore implementations are, naturally, far better and faster than the respective sequential versions. Whereas <samp>MapReduce</samp> can be quite attractive in some domains, it is far from perfect and can not cover every application. However, it can lead to solid parallel efficiency with less LOC than the <samp>PThread</samp> implementations, and in some cases can outperform PThreads by far, if the <samp>MapReduce</samp> implementation can benefit from the application's structure.
...
- Similar to the <samp>Phoenix</samp> benchmark, every multicore program performs faster than the respective singlecore program, except for the <samp>Canneal</samp> workload with small datasets (*Figure 4.1*). 
  
- Small datasets can always be tricky for parallel programs, for they can have more managing-overhead. While the <samp>Canneal</samp> workload has one of the lowest cache-miss rates, it still performs even worse than the sequential version with small datasets. This might be due to the parallelization-model (*Table 2*), for it is the only workload with an unstructured parallelization-model. However, the dataset might also simply be too small, since the speedup is increasing with the large dataset again.

- The cache-misses of the <samp>Vips</samp> workload exceed those of the sequential version by more than 400% (*Figure 4.2, 5.2*). This workload includes 18 stages, most of them being matrix operations and convolution, which explains the noticeable speedup with more threads. However, with multiple threads an big inputs, every thread can load different data into their cache everytime in random order, which can result in very bad cache locality. If the order of the operations could be optimized towards cache locality, the speedups will be even greater.

- The <samp>Streamcluster</samp> workload achieves a huge bump in speed from shy under 100% with small datasets up to a 450% speedup with large datasets. This is also because the input size is too small. The computations of this workload are very primitive, hence the program is memory bound for small data sizes, and gets more computation bound the bigger the input data gets. With an increasing dimensionality of the input data, the multiple cores can be used much more efficiently, resulting in a tremendously better speed-up.

- The other workloads (<samp>Blackscholes, Fluidanimate, Freqmine</samp>) do follow strictly the trend of increasing their performance the more threads are used and larger the dataset is.

- <samp>Parsec</samp> is a very diverse benchmark suite, with various workloads from different areas. These benchmarks can characterize these programs from different domains depending on their hardware environment. It would be interesting to benchmark these workloads on a machine with more than 8 threads to learn if there is a saturation point of performance regarding the number of threads, or whether the slow speed-up with smaller datasets can be compensated with different hardware architectures.
